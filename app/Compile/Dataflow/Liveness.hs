...
nowLive Nop = id
nowLive (CallIr (Just x) _ params) = flip (foldr insertIfReg) params . Set.delete x
nowLive (CallIr Nothing _ params) = flip (foldr insertIfReg) params
nowLive (Goto _) = id
nowLive (GotoIfNot _ (Reg x)) = Set.insert x
nowLive (GotoIfNot _ (Imm _)) = id
nowLive (Return (Imm _)) = const Set.empty
nowLive (Return (Reg x)) = const $ Set.singleton x
nowLive (reg :<-$ (a, _, c, _)) = Set.insert a . maybe id Set.insert c . Set.delete reg
nowLive ((a, _, c, _) :$<- reg) = insertIfReg reg . Set.insert a . maybe id Set.insert c
nowLive (AssertBounds a b) = Set.insert a . Set.insert b
